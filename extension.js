...
const UpdateInterval = vscode.workspace
    .getConfiguration()
    .get("weather.updateInterval");
const ShowLifeIndex = vscode.workspace
    .getConfiguration()
    .get("weather.showLifeIndex");
const barPosition =
    vscode.workspace.getConfiguration().get("weather.position") == "left"
...
function activate(context) {
    let disposable = vscode.commands.registerCommand(
        "extension.weather",
        async function (position = barPosition) {
            ...
            if (locationId) {
                ...
                if (ShowLifeIndex) {
                    ...
                    barLife.tooltip = `生活指数等级: ${lifeIndex.level} ${
                        lifeIndex.text || ""
                    }`;
                    barLife.command = {
                        title: "open",
    ...
    return new Promise(async (resolve) => {
        ...
        if (result.data.code === "200") {
            const detailLocation = await vscode.window.showQuickPick(
                result.data.location.map((v) => v.name + "-" + v.id),
                {
                    placeHolder: "选择一个地区",
                }
            );
